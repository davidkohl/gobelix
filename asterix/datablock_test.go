...
// mockDSI is a mock data source identifier for testing
type mockDSI struct {
	MockDataItem
	sac uint8
	sic uint8
...
// String implements String() for mockDSI to return SAC/SIC format
func (m *mockDSI) String() string {
	return fmt.Sprintf("SAC: %d, SIC: %d", m.sac, m.sic)
}

func TestExtractDataSourceIdentifier(t *testing.T) {
	// Create a mock data item that implements String() returning SAC/SIC format
	mockItem := &mockDSI{
		MockDataItem: MockDataItem{id: "I021/010", data: []byte{0x19, 0x0A}, fixedLen: 2},
		sac:          25,
		sic:          10,
	}

	// Create a record and add the data source identifier
	dataBlock, _, err := setupTestDataBlock()
	if err != nil {
		t.Fatalf("Failed to set up test: %v", err)
	}
	...
	// Add the mock DSI
	err = record.SetDataItem("I021/010", mockItem)
	if err != nil {
		t.Fatalf("Failed to set data item: %v", err)
	}

	// Test extraction
	sac, sic, found := extractDataSourceIdentifier(record)
	if !found {
		t.Error("extractDataSourceIdentifier should find the data source identifier")
	}
	if sac != 25 {
		t.Errorf("Expected SAC=25, got %d", sac)
	}
	if sic != 10 {
		t.Errorf("Expected SIC=10, got %d", sic)
	}

	// Test with record that has no DSI
	record2, err := NewRecord(dataBlock.Category(), dataBlock.UAP())
	if err != nil {
		t.Fatalf("Failed to create record: %v", err)
	}
	_, _, found = extractDataSourceIdentifier(record2)
	if found {
		t.Error("extractDataSourceIdentifier should not find DSI when it doesn't exist")
	}
}

func TestValidateSACandSICConsistency(t *testing.T) {
	dataBlock, _, err := setupTestDataBlock()
	if err != nil {
		t.Fatalf("Failed to set up test: %v", err)
	}

	// Helper to create mock DSI
	createMockDSI := func(sac, sic uint8) DataItem {
		return &mockDSI{
			MockDataItem: MockDataItem{id: "I021/010", data: []byte{sac, sic}, fixedLen: 2},
			sac:          sac,
			sic:          sic,
		}
	}

	// Test case 1: Single record - should pass
	record1, _ := NewRecord(dataBlock.Category(), dataBlock.UAP())
	record1.SetDataItem("I021/010", createMockDSI(25, 10))
	record1.SetDataItem("I021/040", &MockDataItem{id: "I021/040", data: []byte{0xCC}, fixedLen: 1})
	dataBlock.AddRecord(record1)

	err = dataBlock.validateSACandSICConsistency()
	if err != nil {
		t.Errorf("Single record should pass validation: %v", err)
	}

	// Test case 2: Multiple records with same SAC/SIC - should pass
	record2, _ := NewRecord(dataBlock.Category(), dataBlock.UAP())
	record2.SetDataItem("I021/010", createMockDSI(25, 10))
	record2.SetDataItem("I021/040", &MockDataItem{id: "I021/040", data: []byte{0xDD}, fixedLen: 1})
	dataBlock.AddRecord(record2)

	err = dataBlock.validateSACandSICConsistency()
	if err != nil {
		t.Errorf("Records with same SAC/SIC should pass validation: %v", err)
	}

	// Test case 3: Multiple records with different SAC/SIC - should fail
	record3, _ := NewRecord(dataBlock.Category(), dataBlock.UAP())
	record3.SetDataItem("I021/010", createMockDSI(100, 5)) // Different SAC/SIC!
	record3.SetDataItem("I021/040", &MockDataItem{id: "I021/040", data: []byte{0xEE}, fixedLen: 1})
	dataBlock.AddRecord(record3)

	err = dataBlock.validateSACandSICConsistency()
	if err == nil {
		t.Error("Records with different SAC/SIC should fail validation")
	} else {
		// Check that error message contains diagnostic info
		errStr := err.Error()
		if !strings.Contains(errStr, "SAC=25, SIC=10") || !strings.Contains(errStr, "SAC=100, SIC=5") {
			t.Errorf("Error message should contain SAC/SIC values, got: %s", errStr)
		}
		if !strings.Contains(errStr, "misalignment") {
			t.Errorf("Error message should mention misalignment, got: %s", errStr)
		}
	}
}
