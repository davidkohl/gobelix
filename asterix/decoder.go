...
func (d *Decoder) DecodeAll(data []byte) ([]*DataBlock, error) {
	// Pre-allocate results based on typical message size (~50 bytes average)
	estimatedMessages := len(data) / 50
	if estimatedMessages < 1 {
		estimatedMessages = 1
	}
	results := make([]*DataBlock, 0, estimatedMessages)
	offset := 0
...
func (d *Decoder) ExtractMessages(data []byte) ([][]byte, error) {
	// Pre-allocate based on typical message size (~50 bytes average)
	estimatedMessages := len(data) / 50
	if estimatedMessages < 1 {
		estimatedMessages = 1
	}
	messages := make([][]byte, 0, estimatedMessages)
	offset := 0
