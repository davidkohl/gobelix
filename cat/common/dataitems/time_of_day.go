...
func (t *TimeOfDay) Decode(buf *bytes.Buffer) (int, error) {
	...
	if n != 3 {
		return n, fmt.Errorf("%w: need 3 bytes for TimeOfDay, have %d", asterix.ErrBufferTooShort, n)
	}
...
func (t *TimeOfDay) Validate() error {
	...
	if t.Time < 0 || t.Time >= 86400 {
		return fmt.Errorf("time of day must be 0-86400 seconds, got %.3f", t.Time)
	}
	return nil
}
