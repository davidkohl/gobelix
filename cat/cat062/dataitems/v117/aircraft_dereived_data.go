...
func (a *AircraftDerivedData) Decode(buf *bytes.Buffer) (int, error) {
	...
	if len(fspecBytes) > 0 {
		...
		if (fspecBytes[0] & 0x04) != 0 {
			...
			// Extract altitude in two's complement form (14 bits: bits 13-0)
			altVal := int16(uint16(data[0]&0x3F)<<8 | uint16(data[1]))
			// Sign extend from 14 bits to 16 bits
			if (altVal & 0x2000) != 0 { // Check bit 13 (sign bit of 14-bit value)
				altVal |= ^0x3FFF // Set upper bits for negative
			}
		...
		if (fspecBytes[0] & 0x02) != 0 {
			...
			// Extract altitude in two's complement form (13 bits: bits 12-0)
			altVal := int16(uint16(data[0]&0x1F)<<8 | uint16(data[1]))
			// Sign extend from 13 bits to 16 bits
			if (altVal & 0x1000) != 0 { // Check bit 12 (sign bit of 13-bit value)
				altVal |= ^0x1FFF // Set upper bits for negative
			}
	...
	if len(fspecBytes) > 1 {
		...
		if (fspecBytes[1] & 0x40) != 0 {
			...
			for i := 0; i < int(rep); i++ {
				...
				// Parse altitude (10ft resolution, 16-bit signed)
				altVal := int16(uint16(data[1])<<8 | uint16(data[2]))
				point.Altitude = float64(altVal) * 10.0

				// Parse latitude (180/2^23 degrees resolution, 24-bit signed)
				latVal := int32(uint32(data[3])<<16 | uint32(data[4])<<8 | uint32(data[5]))
				// Sign extend from 24 bits to 32 bits
				if (latVal & 0x800000) != 0 { // Check bit 23 (sign bit of 24-bit value)
					latVal |= ^0xFFFFFF // Set upper bits for negative
				}
				point.Latitude = float64(latVal) * 180.0 / float64(1<<23)

				// Parse longitude (180/2^23 degrees resolution, 24-bit signed)
				lonVal := int32(uint32(data[6])<<16 | uint32(data[7])<<8 | uint32(data[8]))
				// Sign extend from 24 bits to 32 bits
				if (lonVal & 0x800000) != 0 { // Check bit 23 (sign bit of 24-bit value)
					lonVal |= ^0xFFFFFF // Set upper bits for negative
				}
...
// using the ICAO Annex 10 6-bit character encoding (uses sixBitToASCII from target_identification.go)
func parseTargetIdentification(data []byte) string {
	...
	// Convert to ASCII using ICAO lookup table
	result := make([]byte, 8)
	for i, c := range chars {
		if int(c) < len(sixBitToASCII) {
			ch := sixBitToASCII[c]
			if ch == '#' {
				result[i] = ' ' // Replace invalid codes with space
			} else {
				result[i] = ch
			}
		} else {
			result[i] = ' ' // Default to space for out-of-range values
		}
	}
...
func encodeTargetIdentification(ident string) []byte {
	...
	// Convert to 6-bit representation using lookup table
	chars := make([]byte, 8)
	for i := 0; i < 8; i++ {
		ch := ident[i]
		found := false
		for j, c := range sixBitToASCII {
			if c == ch {
				chars[i] = byte(j)
				found = true
				break
		...
		if !found {
			chars[i] = 32 // Default to space (code 32) for invalid characters
		}
	}
