...
func (t *TrackStatus) Decode(buf *bytes.Buffer) (int, error) {
	...
	// Parse first octet
	// Per ASTERIX CAT062 spec: MON(8) SPI(7) MRH(6) SRC(5-3) CNF(2) FX(1)
	t.MON = (b & 0x80) != 0 // bit 8
	t.SPI = (b & 0x40) != 0 // bit 7
	t.MRH = (b & 0x20) != 0 // bit 6
	t.SRC = (b & 0x1C) >> 2 // bits 5-3 (0x1C = 0b00011100)
...
func (t *TrackStatus) Encode(buf *bytes.Buffer) (int, error) {
	...
	b |= (t.SRC & 0x07) << 2 // bits 5-3 (shift by 2 to position at bits 4-2)
	if t.CNF {
		b |= 0x02 // bit 2
	}
