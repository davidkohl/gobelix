...
func (m *MeasuredFlightLevel) Decode(buf *bytes.Buffer) (int, error) {
	...
	if n != 2 {
		return n, fmt.Errorf("%w: need 2 bytes for measured flight level, have %d", asterix.ErrBufferTooShort, n)
	}
...
func (m *MeasuredFlightLevel) String() string {
	// Convert flight levels to feet (1 FL = 100 ft)
	altitudeInFeet := m.FlightLevel * 100

	return fmt.Sprintf("Measured Flight Level: %.0f ft (FL %.2f)", altitudeInFeet, m.FlightLevel)
}
