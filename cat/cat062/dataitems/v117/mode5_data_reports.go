...
func (m *Mode5DataReports) Decode(buf *bytes.Buffer) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("reading mode 5 data reports primary subfield: %w", err)
	}
	bytesRead++
	m.Data = append(m.Data, primaryByte)

	// Process each subfield based on bits in the primary subfield
	// SUM: bit-8 Subfield #1: Mode 5 Summary
	if (primaryByte & 0x80) != 0 {
		subfieldData, err := buf.ReadByte()
		if err != nil {
			return bytesRead, fmt.Errorf("reading mode 5 summary subfield: %w", err)
		}
		bytesRead++
	...
	if (primaryByte & 0x04) != 0 {
		subfieldData, err := buf.ReadByte()
		if err != nil {
			return bytesRead, fmt.Errorf("reading time offset subfield: %w", err)
		}
		bytesRead++
		m.Data = append(m.Data, subfieldData)
	}
	...
	if (primaryByte & 0x02) != 0 {
		subfieldData, err := buf.ReadByte()
		if err != nil {
			return bytesRead, fmt.Errorf("reading X pulse presence subfield: %w", err)
		}
		bytesRead++
		m.Data = append(m.Data, subfieldData)
	}
