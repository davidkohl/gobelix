...
func (t *TrackDataAges) Decode(buf *bytes.Buffer) (int, error) {
	...
	//
	// IMPORTANT: Save the FSPEC length before reading subfields, otherwise
	// appending subfield bytes to t.Data will cause the loop to treat subfields
	// as additional FSPEC octets!
	fspecLen := len(t.Data)
	for byteIdx := 0; byteIdx < fspecLen; byteIdx++ {
