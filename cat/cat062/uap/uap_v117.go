...
import (
	...
	cat062 "github.com/davidkohl/gobelix/cat/cat062/dataitems/v117"
	common "github.com/davidkohl/gobelix/cat/common/dataitems"
)

// UAP062v1_17 implements the User Application Profile for ASTERIX Category 062 version 1.17
type UAP062v1_17 struct {
	*asterix.BaseUAP
}

// NewUAP062v1_17 creates a new instance of the Category 062 version 1.17 UAP
func NewUAP117() (*UAP062v1_17, error) {
	base, err := asterix.NewBaseUAP(asterix.Cat062, "1.17", cat062v117Fields)
	if err != nil {
		return nil, err
	}
...
func (u *UAP062v1_17) CreateDataItem(id string) (asterix.DataItem, error) {
	switch id {
	case "I062/010":
...
func (u *UAP062v1_17) Validate(items map[string]asterix.DataItem) error {
	// First do base validation (mandatory fields)
	if err := u.BaseUAP.Validate(items); err != nil {
		return err
	}

	// In version 1.17, mandatory items are:
	// - I062/010 Data Source Identifier
	// - I062/040 Track Number
	// - I062/070 Time Of Track Information
	// - I062/080 Track Status

	// Check for the mandatory items
	_, dataSourceExists := items["I062/010"]
	_, trackNumberExists := items["I062/040"]
	_, timeOfTrackExists := items["I062/070"]
	_, trackStatusExists := items["I062/080"]

	if !dataSourceExists || !trackNumberExists || !timeOfTrackExists || !trackStatusExists {
		return fmt.Errorf("%w: missing mandatory field(s)", asterix.ErrInvalidMessage)
	}

	return nil
}

// cat062v117Fields defines the complete UAP for Category 062 version 1.17
var cat062v117Fields = []asterix.DataField{
	{
		FRN:         1,
		DataItem:    "I062/010",
