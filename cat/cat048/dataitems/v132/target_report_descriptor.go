...
func (t *TargetReportDescriptor) Decode(buf *bytes.Buffer) (int, error) {
	...
	if fx {
		...
		t.ME = (b & 0x10) != 0  // bit 5
		t.MI = (b & 0x08) != 0  // bit 4
		t.FOE = (b >> 1) & 0x03 // bits 3-2 (mask 0x06, shifted right 1)
		fx = (b & 0x01) != 0    // bit 1 (FX)
		...
		if fx {
			...
			// Third Extension (per ASTERIX CAT048 spec page 18)
			// Octet: ACASXV_EP(bit8) ACASXV_VAL(bits7-4) POXPR_EP(bit3) POXPR_VAL(bit2) FX(bit1)
			if fx {
				t.extensions = 3
				b, err = buf.ReadByte()
				if err != nil {
				...
				t.ACASXV_VAL = (b >> 3) & 0x0F // bits 7-4: shift right 3, mask 4 bits
				t.POXPR_EP = (b & 0x04) != 0   // bit 3 = 0x04
				t.POXPR_VAL = (b & 0x02) != 0  // bit 2 = 0x02
				fx = (b & 0x01) != 0           // bit 1 (FX)
				...
				if fx {
					...
					// Fifth Extension (per ASTERIX CAT048 spec page 20)
					// Octet: IRMXPR(bits8-7) IRMACT(bits6-5) spare(bits4-2) FX(bit1)
					if fx {
						t.extensions = 5
						b, err = buf.ReadByte()
						if err != nil {
						...
						// bits 4-2 are spare
						fx = (b & 0x01) != 0 // bit 1 (FX) - can indicate future extensions

						// Handle any additional future extensions (skip unknown extension bytes)
						for fx {
							t.extensions++
							b, err = buf.ReadByte()
							if err != nil {
...
func (t *TargetReportDescriptor) Encode(buf *bytes.Buffer) (int, error) {
	...
	if t.extensions > 0 {
		...
		b |= (t.FOE & 0x03) << 1 // bits 3-2 (values 0x02 and 0x04)
		if t.extensions > 1 {
			b |= 0x01 // bit 1 (FX)
		}
		...
		if t.extensions > 1 {
			...
			if t.extensions > 2 {
				...
				b |= (t.ACASXV_VAL & 0x0F) << 3 // bits 7-4: shift left 3
				if t.POXPR_EP {
					b |= 0x04 // bit 3 = 0x04
				}
				if t.POXPR_VAL {
