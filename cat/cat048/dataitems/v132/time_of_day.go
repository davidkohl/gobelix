...
func (t *TimeOfDay) Decode(buf *bytes.Buffer) (int, error) {
	...
	if n != 3 {
		return n, fmt.Errorf("%w: need 3 bytes for time of day, have %d", asterix.ErrBufferTooShort, n)
	}
	...
	// Note: We don't validate during decode for interoperability.
	// Some radar implementations may send invalid time values due to clock issues.
	// The caller can use Validate() for strict validation if needed.
	return n, nil
}
...
func (t *TimeOfDay) Encode(buf *bytes.Buffer) (int, error) {
	...
	data := [3]byte{
		byte(counts >> 16), // Most significant 8 bits
		byte(counts >> 8),  // Middle 8 bits
		byte(counts),       // Least significant 8 bits
	}
