...
func (b *BDSRegisterData) Decode(buf *bytes.Buffer) (int, error) {
	...
	// Sanity check: More than 16 BDS registers is unlikely in practice.
	// Very high values indicate buffer misalignment from earlier items.
	const maxReasonableREP = 16
	if repetitions > maxReasonableREP {
		return bytesRead, fmt.Errorf("BDS register REP=%d is unreasonably high (max %d), likely buffer misalignment", repetitions, maxReasonableREP)
	}

	// Check if we have enough data before attempting to read
	expectedBytes := repetitions * 8 // Each register is 7 bytes data + 1 byte BDS code
	if buf.Len() < expectedBytes {
		return bytesRead, fmt.Errorf("%w: need %d bytes for BDS registers (%d registers), have %d", asterix.ErrBufferTooShort, expectedBytes, repetitions, buf.Len())
	}
	...
	for i := 0; i < repetitions; i++ {
		...
		if m != 7 {
			return bytesRead, fmt.Errorf("%w: need 7 bytes for BDS register, have %d", asterix.ErrBufferTooShort, m)
		}
