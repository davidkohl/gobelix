...
func (r *RadarPlotCharacteristics) Decode(buf *bytes.Buffer) (int, error) {
	...
	// NOTE: Bit 1 (FX) is intentionally IGNORED for I048/130 v1.32
	//
	// According to EUROCONTROL ASTERIX Cat048 v1.32 specification,
	// I048/130 defines exactly 7 subfields (SRL, SRR, SAM, PRL, PAM, RPD, APD)
	// which all fit in the primary octet. No extension octets are defined.
	//
	// However, some radar implementations set the FX bit incorrectly, causing
	// the decoder to read the next field's data as an extension octet. This
	// leads to buffer misalignment and spurious SAC/SIC values in subsequent records.
	//
	// The correct behavior per spec is to NOT process FX for I048/130 v1.32.
	// If future versions add extension subfields, a separate implementation
	// should be created for that version.
	//
	// This fixes the "weird sac bug" where multi-record CAT048 messages
	// showed different SAC/SIC values due to buffer misalignment.
