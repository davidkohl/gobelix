// dataitems/cat048/v117/radial_doppler_speed.go
package v117
...
// RadialDopplerSpeed implements I048/120 for version 1.17
// Information on the Doppler Speed of the target report.
// Note: v1.17 has NO FX bit in primary subfield (bit 1 is spare, set to 0)
...
func (r *RadialDopplerSpeed) Decode(buf *bytes.Buffer) (int, error) {
	...
	// Primary subfield (v1.17): CAL(bit8) RDS(bit7) spare(bits6-1)
	// Note: In v1.17, there is NO FX bit - bits 6-1 are all spare
	var primary [1]byte
	...
	// bits 6-1 are spare in v1.17 (no FX bit!)

	// Note: According to v1.17 spec, only one secondary subfield shall be present.
	// However, we allow both for interoperability.
...
func (r *RadialDopplerSpeed) Encode(buf *bytes.Buffer) (int, error) {
	...
	// Primary subfield (v1.17): NO FX bit
	primary := byte(0)
	if r.CAL {
		primary |= 0x80 // bit 8
	}
	...
	// bits 6-1 are spare (no FX in v1.17)
