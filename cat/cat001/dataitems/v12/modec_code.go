...
func (m *ModeCCode) Decode(buf *bytes.Buffer) (int, error) {
	...
	n, err := buf.Read(data[:])
	if err != nil || n != 2 {
		return n, fmt.Errorf("%w: need 2 bytes for Mode-C code, have %d", asterix.ErrBufferTooShort, n)
	}
...
func (m *ModeCCode) Encode(buf *bytes.Buffer) (int, error) {
	...
	if err := buf.WriteByte(octet1); err != nil {
		return 0, fmt.Errorf("writing Mode-C code byte 1: %w", err)
	}
	if err := buf.WriteByte(octet2); err != nil {
		return 1, fmt.Errorf("writing Mode-C code byte 2: %w", err)
	}
	return 2, nil
}
