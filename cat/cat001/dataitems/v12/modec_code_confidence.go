...
func (m *ModeCCodeConfidence) Decode(buf *bytes.Buffer) (int, error) {
	...
	n, err := buf.Read(data[:])
	if err != nil || n != 4 {
		return n, fmt.Errorf("%w: need 4 bytes for Mode-C code confidence, have %d", asterix.ErrBufferTooShort, n)
	}
...
func (m *ModeCCodeConfidence) Encode(buf *bytes.Buffer) (int, error) {
	...
	if err := buf.WriteByte(octet1); err != nil {
		return 0, fmt.Errorf("writing Mode-C code confidence byte 1: %w", err)
	}
	if err := buf.WriteByte(octet2); err != nil {
		return 1, fmt.Errorf("writing Mode-C code confidence byte 2: %w", err)
	}
	if err := buf.WriteByte(octet3); err != nil {
		return 2, fmt.Errorf("writing Mode-C code confidence byte 3: %w", err)
	}
	if err := buf.WriteByte(octet4); err != nil {
		return 3, fmt.Errorf("writing Mode-C code confidence byte 4: %w", err)
	}
