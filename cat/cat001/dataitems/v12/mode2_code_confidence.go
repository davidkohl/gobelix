...
// Mode2CodeConfidence represents I001/060 - Mode-2 Code Confidence Indicator
...
func (m *Mode2CodeConfidence) Decode(buf *bytes.Buffer) (int, error) {
	...
	n, err := buf.Read(data[:])
	if err != nil || n != 2 {
		return n, fmt.Errorf("%w: need 2 bytes for Mode-2 code confidence, have %d", asterix.ErrBufferTooShort, n)
	}
...
func (m *Mode2CodeConfidence) Encode(buf *bytes.Buffer) (int, error) {
	...
	if err := buf.WriteByte(octet1); err != nil {
		return 0, fmt.Errorf("writing Mode-2 code confidence byte 1: %w", err)
	}
	if err := buf.WriteByte(octet2); err != nil {
		return 1, fmt.Errorf("writing Mode-2 code confidence byte 2: %w", err)
	}
	return 2, nil
}

func (m *Mode2CodeConfidence) String() string {
	return fmt.Sprintf("Mode-2 Confidence Indicator")
}
