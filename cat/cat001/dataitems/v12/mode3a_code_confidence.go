...
func (m *Mode3ACodeConfidence) Decode(buf *bytes.Buffer) (int, error) {
	...
	n, err := buf.Read(data[:])
	if err != nil || n != 2 {
		return n, fmt.Errorf("%w: need 2 bytes for Mode-3/A code confidence, have %d", asterix.ErrBufferTooShort, n)
	}
...
func (m *Mode3ACodeConfidence) Encode(buf *bytes.Buffer) (int, error) {
	...
	if err := buf.WriteByte(octet1); err != nil {
		return 0, fmt.Errorf("writing Mode-3/A code confidence byte 1: %w", err)
	}
	if err := buf.WriteByte(octet2); err != nil {
		return 1, fmt.Errorf("writing Mode-3/A code confidence byte 2: %w", err)
	}
	return 2, nil
}
