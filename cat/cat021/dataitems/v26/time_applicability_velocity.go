...
func (t *TimeOfApplicabilityVelocity) Decode(buf *bytes.Buffer) (int, error) {
	...
	if n != 3 {
		return n, fmt.Errorf("%w: need 3 bytes for time of applicability velocity, have %d", asterix.ErrBufferTooShort, n)
	}
...
func (t *TimeOfApplicabilityVelocity) String() string {
