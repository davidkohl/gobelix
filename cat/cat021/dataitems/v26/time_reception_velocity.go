...
func (t *TimeOfMessageReceptionVelocity) Decode(buf *bytes.Buffer) (int, error) {
	...
	n, err := buf.Read(data[:])
	if err != nil {
		return n, fmt.Errorf("reading time of message reception velocity: %w", err)
	}
	if n != 3 {
		return n, fmt.Errorf("%w: need 3 bytes for time of message reception velocity, have %d", asterix.ErrBufferTooShort, n)
	}
...
func (t *TimeOfMessageReceptionVelocity) Encode(buf *bytes.Buffer) (int, error) {
	...
	n, err := buf.Write(b[:])
	if err != nil {
		return n, fmt.Errorf("writing time of message reception velocity: %w", err)
	}
	return n, nil
}
