// dataitems/cat021/time_reception_velocity_high.go
package v26
...
// High precision variant showing fraction of the second for velocity reception time
...
func (t *TimeOfMessageReceptionVelocityHigh) Decode(buf *bytes.Buffer) (int, error) {
	...
	n, err := buf.Read(data[:])
	if err != nil {
		return n, fmt.Errorf("reading high precision velocity time: %w", err)
	}
	if n != 4 {
		return n, fmt.Errorf("%w: need 4 bytes for high precision velocity time, have %d", asterix.ErrBufferTooShort, n)
	}
...
func (t *TimeOfMessageReceptionVelocityHigh) Encode(buf *bytes.Buffer) (int, error) {
	...
	n, err := buf.Write(b[:])
	if err != nil {
		return n, fmt.Errorf("writing high precision velocity time: %w", err)
	}
	return n, nil
}
