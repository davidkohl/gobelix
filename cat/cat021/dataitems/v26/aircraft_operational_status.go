...
func (a *AircraftOperationalStatus) String() string {
	var parts []string
	...
	if a.TC > 0 {
		parts = append(parts, fmt.Sprintf("TC%d", a.TC))
	}
	if a.TS {
		parts = append(parts, "TS")
	}
	if a.ARV {
		parts = append(parts, "ARV")
	}
	if a.CDTIA {
		parts = append(parts, "CDTIA")
	}
	if a.NotTCAS {
		parts = append(parts, "NotTCAS")
	}
	if a.SA {
